/**
 * ARQUIVO: scripts/customizacao-abas.js
 * PAPEL: Gerenciador de Preferências de Navegação (UX) e Orquestrador de Cores
 * VERSÃO: 8.0 - Integração Constitucional com Router & Temas
 */
...
const styles = `
  ...
  .ag-section-header-btn.is-active .ag-section-text { color: var(--tema-cor, #8A2BE2); text-decoration: underline; text-underline-offset: 4px; }
  ...
  .ag-card.is-selected { background: #fff; border-color: var(--tema-cor, #8A2BE2); color: var(--tema-cor, #8A2BE2); box-shadow: inset 0 0 0 1px var(--tema-cor, #8A2BE2); font-weight: 700; }
...
/**
 * 🎨 ORQUESTRADOR DE CORES (Ponto 3)
 * Sincroniza a cor da seção com o CSS Global
 */
function aplicarCorTema(idSecao) {
    const secao = CATALOGO.find(s => s.id === idSecao || s.itens.some(i => i.id === idSecao));
    if (secao) {
        document.documentElement.style.setProperty('--tema-cor', secao.cor);
    }
}

function getOrder(){
  const saved = load(CONFIG.KEYS.ORDER, null);
  return saved || ['manchetes', 'analises', 'futebol', 'tecnologia'];
}
...
/* ===========================
   RENDERIZAÇÃO BARRA HORIZONTAL (ROUTER SYNC)
=========================== */
function renderBar(){
  ...
  order.forEach(id => {
    ...
    // Marca ativo se for a seção atual no Router
    if (window.appNavigationState?.secaoAtiva === id) {
        btn.classList.add('active');
        aplicarCorTema(id); // Já nasce com a cor certa
    }
    ...
    btn.onclick = () => {
      ...
      // 🚀 NAVEGAÇÃO OFICIAL (Ponto 8)
      if(window.router) {
          const secaoPai = CATALOGO.find(s => s.itens.some(i => i.id === id) || s.id === id);
          window.router.irPara(secaoPai ? secaoPai.id : id);
      }
    };
    bar.appendChild(btn);
  });
...
/* ===========================
   AÇÕES & NOTIFICAÇÕES (CUSTOM EVENTS)
=========================== */
function toggleItem(id, label){
  ...
  // Notifica mudança de layout (Ponto 2)
  window.dispatchEvent(new CustomEvent('ui:layout_changed', { detail: { order } }));
}
